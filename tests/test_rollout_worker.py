...
def test_rollout_worker():
    ...
    # Shared memory counter for tracking completed episodes
    episodes_completed = mp.Value("i", 0)
    ...
    # Run rollouts
    num_episodes = 3
    observations, actions, rewards, episode_lengths = worker.run_episodes(num_episodes=num_episodes)
    ...
    assert episodes_completed.value == num_episodes, \
        f"episodes_completed should be {num_episodes}, but got {episodes_completed.value}"